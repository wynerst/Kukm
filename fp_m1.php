...
//	'CONCAT(YEAR(p.finaldate),\' \',MONTHNAME(p.finaldate)) as \'Bulan\'',
...
$agr_tahun->setSQLColumn('p.periode as \'Periode\'',
	'CONCAT(YEAR(p.finaldate),\' \',MONTHNAME(p.finaldate)) as \'Bulan\'',
	'format(sum(c.c11)/1000,2) AS \'Aktiva Lancar\'',
	'format(sum(c.c12)/1000,2) AS \'Investasi Jangka Panjang\'',
	'format(sum(c.c13)/1000,2) AS \'Aktiva Tetap\'',
	'format(sum(c.c14)/1000,2) AS \'Aktiva Lain\'',
	'format(sum(c.c21)/1000,2) AS \'Kewajiban Lancar\'',
	'format(sum(c.c22)/1000,2) AS \'Kewajiban Jangka Panjang\'',
	'format(sum(c.c3)/1000,2) AS \'Equitas\'');

$agr_tahun->setSQLorder('p.finaldate DESC');
$agr_tahun->sql_group_by = 'YEAR(p.finaldate), MONTH(p.finaldate)';
