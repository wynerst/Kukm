...
if (isset($_POST['doImport'])) {
  ...
  if (!$_FILES['importFile']['name']) {
    utility::jsAlert('Please select the file to import!');
    exit();
  } else if (empty($_POST['fieldSep']) OR empty($_POST['fieldEnc'])) {
    utility::jsAlert('Required fields (*)  must be filled correctly!');
    exit();
  } else {
    $start_time = time();
    // set PHP time limit
    ...
    if ($upload_status != UPLOAD_SUCCESS) {
        utility::jsAlert('Upload failed! File type not allowed or the size is more than'.($sysconf['max_upload']/1024).' MB'); //mfc
        exit();
    }
    // uploaded file path
    $uploaded_file = $temp_dir.DIRECTORY_SEPARATOR.$_FILES['importFile']['name'];
...
<div class="menuBoxInner importIcon">
	<div class="per_title">
    <h2><?php echo 'Import Tool'; ?></h2>
    </div>
    <div class="infoBox">
    <?php echo 'Import for bibliographics data from CSV file. For guide on CVS fields order and format please refer to documentation or visit <a href="http://slims.web.id" target="_blank">Official Website</a>'; ?>
	</div>
</div>
</fieldset>
...
$form->submit_button_attr = 'name="doImport" value="Import Now" class="button"';
...
$form->addAnything('File To Import', $str_input);
// field separator
$form->addTextField('text', 'fieldSep', 'Field Separator*', ''.htmlentities(',').'', 'style="width: 10%;" maxlength="3"');
//  field enclosed
$form->addTextField('text', 'fieldEnc', 'Field Enclosed With*', ''.htmlentities('"').'', 'style="width: 10%;"');
// number of records to import
$form->addTextField('text', 'recordNum', 'Number of Records To Export (0 for all records)', '0', 'style="width: 10%;"');
// records offset
$form->addTextField('text', 'recordOffset', 'Start From Record', '1', 'style="width: 10%;"');
// output the form
echo $form->printOut();
